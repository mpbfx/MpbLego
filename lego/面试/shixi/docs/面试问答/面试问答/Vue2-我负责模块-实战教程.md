...
## 11. 补充：主导职责、量化结果、复盘与分时长讲法

### 11.1 主导职责（可直接说）

本教程覆盖模块均由我主导：页面架构设计、`asyncData` 数据流、`utils/resource/*` 复用体系、性能优化与交互体验落地。

### 11.2 量化结果（请按真实数据替换）

- SSR 首屏：首屏数据可用时间从 `X ms` 到 `Y ms`。
- 复用改造：重复代码行数下降 `X%`，新卡片接入时间从 `X` 天到 `Y` 天。
- 列表体验：局部更新后用户操作中断率从 `X%` 到 `Y%`。
- 页面稳定性：mounted 端浏览器 API 报错次数从 `X` 到 `Y`。

### 11.3 故障复盘卡片

1) `asyncData` 串行请求导致首屏慢：改为并发后显著下降。  
2) 浏览器 API 放在 SSR 生命周期导致报错：统一收口到 mounted 并加资源释放。  
3) 大组件膨胀导致改动风险高：改为工厂函数模块化后职责更清晰。

### 11.4 分时长回答（背诵版）

- 30 秒：
  我主导把 Vue2/Nuxt2 的核心能力落在三件事：SSR 首屏、业务逻辑复用和高频交互体验。通过 `asyncData`、工厂函数和局部更新，把性能与可维护性同时做上去。

- 90 秒：
  我负责的模块里，先用 `asyncData` 做首屏并发预取，再把复杂业务拆进 `utils/resource/*` 统一复用，最后在列表与详情页做交互优化（URL 状态、局部更新、IntersectionObserver）。这三层组合让页面更快、代码更稳、迭代更轻。

- 3 分钟：
  先讲场景，再讲机制：SSR 用于首屏与路由直达；复用用工厂函数替代 mixin；体验用局部更新和懒加载。最后补充落地结果（性能、缺陷率、迭代效率）。
